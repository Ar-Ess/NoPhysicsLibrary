...
void Physics::DebugBools(DynamicBody* dB, float dt)
{
	...
	for (it = bodies.begin(); it != bodies.end(); ++it)
	{
		...
		if (body->bodyType == DYNAMIC_BODY)
		{
			...
			{
				render->DrawRectangle({ xOff, 10 - yOff, 25, 15 }, { 255, 0, 0, 255 }, true, false);
				ground = true;
			}
			
			if (dB->onLeftWall && !lWall)
			{
				render->DrawRectangle({ xOff, 35 - yOff, 25, 15 }, { 0, 255, 0, 255 }, true, false);
				lWall = true;
			}

			if (dB->onRightWall && !rWall)
			{
				render->DrawRectangle({ xOff, 60 - yOff, 25, 15 }, { 255, 255, 0, 255 }, true, false);
				rWall = true;
			}
			
			if (dB->onRoof && !roof)
			{
				render->DrawRectangle({ xOff, 85 - yOff, 25, 15 }, { 0, 0, 255, 255 }, true, false);
				roof = true;
			}

			if (dB->onAir && !air)
			{
				render->DrawRectangle({ xOff, 110 - yOff, 25, 15 }, { 225, 105, 55, 255 }, true, false);
				air = true;
			}

			if (dB->onJump && !jump)
			{
				render->DrawRectangle({ xOff1, 10 - yOff, 25, 15 }, { 0, 255, 255, 255 }, true, false);
				jump = true;
			}

			if (dB->onDoubleJump && !doubleJump)
			{
				render->DrawRectangle({ xOff1, 35 - yOff, 25, 15 }, { 255, 0, 255, 255 }, true, false);
				doubleJump = true;
			}

			if (dB->onWallJump && !wallJump)
			{
				render->DrawRectangle({ xOff1, 60 - yOff, 25, 15 }, { 255, 255, 255, 255 }, true, false);
				wallJump = true;
			}

			if (dB->onDash && !dash)
			{
				render->DrawRectangle({ xOff1, 85 - yOff, 25, 15 }, { 225, 55, 105, 255 }, true, false);
				dash = true;
			}

			if (dB->onWater && !water)
			{
				render->DrawRectangle({ xOff1, 110 - yOff, 25, 15 }, { 55, 225, 105, 255 }, true, false);
				water = true;
			}
		}
	}
